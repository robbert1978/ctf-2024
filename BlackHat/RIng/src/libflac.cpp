...
void FlacFile::Dump()
{
  ...
  if (_picture)
  {
    ...
    std::cout << "  Colors: " << _picture->colorCount << std::endl
              << std::endl;
  }
}
...
void FlacFile::parseBlock(uint8_t type, uint32_t size)
{
  switch (type)
  {
  case TYPE_STREAMINFO:
    parseBlockStreamInfo(size);
    break;
  case TYPE_APPLICATION:
    parseBlockApplication(size);
    break;
  case TYPE_SEEKTABLE:
    parseBlockSeekTable(size);
    break;
  case TYPE_VORBIS_COMMENT:
    parseBlockVorbisComment(size);
    break;
  case TYPE_CUESHEET:
    parseBlockCueSheet(size);
    break;
  case TYPE_PICTURE:
    parseBlockPicture(size);
    break;
  default:
    parseBlockSkip(size);
    break;
...
{
  str.resize(size);
  getBuffer(reinterpret_cast<uint8_t *>((char *)str.data()), size);
}
...
const char *FlacFile::Reason()
{
  ...
  case FLAC_SUCCESS:
    return "Success";
  case FLAC_INVALID_MAGIC:
    return "Not a FLAC file";
  case FLAC_INVALID_STREAMINFO:
    return "Invalid stream information";
  case FLAC_INVALID_STREAMINFO_POS:
    return "First block is not stream information";
  case FLAC_INVALID_APPLICATION:
    return "Invalid application";
  case FLAC_INVALID_VORBIS_COMMENT:
    return "Invalid Vorbis comment";
  case FLAC_INVALID_SEEKTABLE:
    return "Invalid seek table";
  case FLAC_INVALID_PICTURE:
