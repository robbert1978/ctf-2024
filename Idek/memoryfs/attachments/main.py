...
class Shell:
    ...
    def __navigate(self, path, striplast=True, cur=None):
        ...
        for token in path:
            ...
            if type(cur.children[token]) == RegularFile:
                ...
                cur, _ = self.__navigate(
                    cur.children[token].target, False, cur)
                if not cur:
                    return None, "No such file or directory"
            else:
    ...
    def create_flag(self):
        if "flag.txt" not in self.root.children:
            self.root.children["flag.txt"] = RegularFile(
                "flag.txt", os.getenv("FLAG") or "idek{fake_flag}")
            return 0, "", ""
    ...
    def _run(self):
        ...
        while True:
            cmd = input(
                "user@memoryFS:/{}$ ".format("/".join(self.displaycwd)))
            if cmd == "exit":
                break
            tokens = cmd.split()
            if len(tokens) == 0:
